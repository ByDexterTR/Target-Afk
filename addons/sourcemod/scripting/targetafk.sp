...
public int Native_IsClientAfk(Handle hPlugin, int numParams)
{
	...
	bool HeAfk = false;
	if (IsValidClient(client) && IsPlayerAlive(client) && gametime[client] + 5 <= GetTime())
	{
		HeAfk = true;
	}
...
{
	for (int i = 1; i <= MaxClients; i++)if (IsValidClient(i) && IsPlayerAlive(i) && gametime[i] + 5 <= GetTime())
	{
